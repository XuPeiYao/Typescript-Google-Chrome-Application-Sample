            ...
            getMediaApiData(mediaId) {
                return __awaiter(this, void 0, Promise, function* () {
                    ...
                    for (var i = 0; i < propertys.length; i++) {
                        ...
                        var value = atob(propertys[i].firstChild.textContent.innerString('[CDATA[', ']]'));
                        if (value)
...
(function (MediaGet) {
    MediaGet.matchRegex = {
        [MediaGet.Extractors.YoutubeExtractor]: /^https?:\/\/www.youtube.com\/watch\?v=.+/,
        [MediaGet.Extractors.XuiteExtractor]: /(^https?:\/\/vlog.xuite.net\/play\/\S+)|(^https?:\/\/m.xuite.net\/vlog\/\S+\/\S+)/
    };
})(MediaGet || (MediaGet = {}));
var MediaGet;
(function (MediaGet) {
    "use strict";
    class ExtractorAdapter extends MediaGet.ExtractorBase {
        constructor(...types) {
            super();
        ...
        getMediaInfosAsync(url) {
            return __awaiter(this, void 0, Promise, function* () {
                for (var i = 0; i < this.supportTypes.length; i++) {
                    var extractor = Object.create(this.supportTypes[i]);
                    if (!extractor.isMatch(url))
                        continue;
                    return yield extractor.getMediaInfosAsync(url);
                }
                return null;
            });
        }
        static getInstance() {
            var result = new ExtractorAdapter();
            result.supportTypes = MediaGet.matchRegex.getKeys();
            return result;
        }
    }
    MediaGet.ExtractorAdapter = ExtractorAdapter;
})(MediaGet || (MediaGet = {}));
